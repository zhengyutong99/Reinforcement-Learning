...
public:
	...
	virtual float estimate(const board& b) const {
		...
		for (int i = 0; i < iso_last; i++) {
			size_t iso_index = indexof(isomorphic[i], b);
			iso_value += operator[](iso_index);
		}
		return iso_value;
	}
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
        ...
        for (size_t i = 0; i < patt.size(); i++) {
            int pattern_value = patt[i];
            int board_value = b.at(pattern_value);
            int shifted_value = board_value << (4 * i);
            index |= shifted_value;
        }
        return index;
	}
...
public:
	 ...
	 */
	float 
	update(const board& b, float u) const {
			...
			if (move->assign(b)) {
				...
				for (int i = 0; i < 16; i++) {
					if (b.at(i) == 0) {
						...
						board tile2 = move->after_state();
						board tile4 = move->after_state();

						tile2.set(i, 1); 
						float score2 = estimate(tile2);
						tile4.set(i, 2);
						float score4 = estimate(tile4);

						total_score += (0.1 * score4 + (1 - 0.1) * score2);
					}
				}
				
				if (num != 0) {
					move->set_value((total_score / num) + move->reward());
				} else {
...
void update_episode(std::vector<state>& path, float alpha = 0.1) const {
	// TODO
	float TD_target = 0, TD_error = 0;
	// float TD_old_target = 0;

		for (int j = path.size() - 1; j > 0; j--) {
			state& action_t_1 = path[j - 1];
			state& action_t = path[j];

			TD_error = TD_target - estimate(action_t.before_state());

			TD_target = action_t_1.reward() + update(action_t.before_state(), alpha * TD_error);
			// TD_old_target = TD_target;
			// TD_target = action_t_1.reward() + lambda*TD_old_target + update(action_t.before_state(), alpha * TD_error)*(1-lambda);
		}

		TD_error = TD_target - estimate(path[0].before_state());
		update(path[0].before_state(), alpha * TD_error);
		// update(path[0].before_state(), alpha * TD_error)*(1-lambda);
}
	 ...
	 */
		...
		void make_statistic(size_t n, const board& b, int score, float& COUNT, int unit = 1000) {
...
int main(int argc, const char* argv[]) {
	...
	tdl.load("/home/zhengyutong/data/Reinforce_Learning/Lab1/340000_demo_weight.bin");
	// tdl.load("/home/zhengyutong/data/Reinforce_Learning/Lab1/05turn0/50000_05turn0_00025_weight.bin");
	...
	for (size_t n = 1; n <= total; n++) {
		...
		tdl.make_statistic(n, b, score, COUNT, 1000);
		
		// std::string basename = "turn0_00025_weight.bin";
		// std::string fullname = "./05turn0/" + std::to_string(n) + "_"+ basename ;
